    ...
    public class ContractMapper : AutoMapper.Profile
    {
        public ContractMapper()
        {
            CreateMap<ContractVM, Contract>()
                ...
                .ForMember(dest => dest.ContractConsultancy, opt => opt.MapFrom(src => $"{src.ContractConsultancy}")).ReverseMap().ForMember(dest => dest.ContractConsultancy, opt => opt.Ignore())
                .ForMember(dest => dest.ContractFinancialAdmin, opt => opt.MapFrom(src => $"{src.ContractFinancialAdmin}")).ReverseMap().ForMember(dest => dest.ContractFinancialAdmin, opt => opt.Ignore())
                .ForMember(dest => dest.VideoAgree, opt => opt.MapFrom(src => $"{src.VideoAgree}")).ReverseMap().ForMember(dest => dest.VideoAgree, opt => opt.Ignore())
